...
\textbf{Synthetic trap tasks (95 tasks).} 80 tasks are programmatically generated across 5 domains (tabular EDA, time-series, recommendation, statistical, text-log), with 16 instances per domain using a fixed seed (seed$=$16). Tasks share structural templates within each domain but use different generated data, producing 70/80 unique answers (87.5\% uniqueness). Near-duplicate questions (identical first 100 chars) exist within domains (55/80) due to shared question structure, but the underlying data and expected answers differ. The remaining 15 tasks are DSBench questions (identified by \texttt{dsbench\_00000001} or \texttt{dsbench\_00000002} prefixes in the task ID) that are structurally similar to synthetic traps but originate from DSBench workbooks. Train/dev/test splits are by task ID, not by template, so the same template appears in all splits---this is intentional, as the goal is to evaluate failure diagnosis, not template memorisation.
...
Each run in the enriched manifest (\texttt{manifest\_v3.csv}, 32 fields) contains:
...
\item \texttt{annotation\_confidence}, \texttt{human\_gold}, \texttt{adjudication\_status}: human annotation fields populated for all 150 gold traces
\end{itemize}

\textbf{Annotation uncertainty.} The release contains 150 human-adjudicated traces. The new 103-trace tranche preserves three independent labels, five blinded QC duplicates per annotator, and consensus adjudications. Raw agreement is Fleiss' $\kappa=.350$ for stage and $.590$ for observability; the revised oracle-assisted classifier achieves 91.3\% accuracy against final gold. Remaining traces retain rule labels and should not be treated as equivalent to human gold.
...
\item Human adjudication covers 150 failures; the revised oracle-assisted classifier reaches 91.3\% accuracy but requires independent held-out validation because its rules were refined after gold error analysis.
