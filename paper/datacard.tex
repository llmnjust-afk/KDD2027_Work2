...
\item \texttt{annotation\_confidence}, \texttt{human\_gold}, \texttt{adjudication\_status}: human annotation fields (pending)
\end{itemize}

\textbf{Annotation uncertainty.} The \texttt{label\_source} field indicates whether a label comes from the rule-based classifier (\texttt{rule}) or is unmatched. The \texttt{annotation\_confidence} and \texttt{human\_gold} fields are currently empty, pending the 47-trace human annotation protocol. Once completed, each trace will have $\geq 3$ independent labels and an adjudicated gold label; the agreement rate will be reported as Fleiss' $\kappa$.
...
\item The rule-based classifier labels are validated only against an LLM judge ($\kappa=0.435$); human gold labels are pending.
