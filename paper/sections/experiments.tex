...
\Cref{tab:main-results} presents the comprehensive results across all five models and all three task suites. DeepSeek-V3 achieves the highest success rate on both synthetic (88.4\%) and UCI real data (94.2\%), while also being the most token-efficient. The corrected SFR reveals a task-dependent pattern: on synthetic trap tasks, SFR is 86--100\% for all models (failures are almost always wrong answers, not crashes); on UCI real data, SFR drops to 0--100\% depending on model; and on DSBench real data, SFR is 19--36\%. This confirms that high SFR is specific to tasks designed to elicit silent failures, not a universal property.

\begin{table*}[t]
\centering
\caption{Comprehensive results across all five models and three task suites (3 runs each). SR = success rate ($\pm$std), SFR = silent-failure rate of failures (corrected classifier, computed only on failed runs), Tok/succ = tokens per successful task, \$/succ = cost per success (USD). ``---'' indicates no successes. Bold marks the best SR per suite.}
\label{tab:main-results}
\small
\begin{tabular}{@{}llcccccccc@{}}
\toprule
& & \multicolumn{3}{c}{Synthetic traps (80 tasks)} & \multicolumn{3}{c}{UCI real (23 tasks)} & \multicolumn{2}{c}{DSBench real (200 tasks)} \\
\cmidrule(lr){3-5} \cmidrule(lr){6-8} \cmidrule(lr){9-10}
Model & Variant & SR (\%) & SFR (\%) & Tok/succ & SR (\%) & SFR (\%) & Tok/succ & SR (\%) & SFR (\%) \\
\midrule
DeepSeek-V3 & baseline & \textbf{88.4}$\pm$1.0 & 100.0 & 1{,}943 & \textbf{94.2}$\pm$2.5 & 0.0 & 2{,}562 & --- & --- \\
DeepSeek-V3 & +verifier & 87.7$\pm$3.7 & 97.1 & 2{,}133 & --- & --- & --- & --- & --- \\
GPT-4o & baseline & 81.4$\pm$0.6 & 100.0 & 1{,}405 & 91.3$\pm$0.0 & 100.0 & 1{,}690 & 3.3 & 82.8 \\
GPT-4o & +verifier & 81.1$\pm$1.8 & 100.0 & 1{,}473 & --- & --- & --- & --- & --- \\
GPT-4o-mini & baseline & 77.9$\pm$1.0 & 85.7 & 2{,}822 & 82.6$\pm$4.4 & 0.0 & 5{,}663 & 0.8 & 36.0 \\
GPT-4o-mini & +verifier & 72.6$\pm$1.0 & 84.6 & 3{,}242 & --- & --- & --- & --- & --- \\
Qwen3-max & baseline & 18.3$\pm$0.6 & 100.0 & 1{,}052 & 0.0 & 0.0 & --- & --- & --- \\
Qwen3-max & +verifier & 43.2$\pm$3.8 & 99.4 & 1{,}715 & --- & --- & --- & --- & --- \\
DeepSeek-R1 & baseline & 0.0$\pm$0.0 & 100.0 & --- & 0.0 & 0.0 & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{table*}

\Cref{fig:model-comparison} visualises the success-rate and SFR comparison on the synthetic suite. On UCI real data, the SFR pattern shifts: DeepSeek-V3 and GPT-4o-mini show SFR$=$0\% (all failures are loud execution errors), while GPT-4o shows SFR$=$100\% (its few failures are all silent). This model-dependent bifurcation on real data confirms that silent failure is not purely a synthetic-task artifact.
